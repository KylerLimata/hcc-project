    ...
    with tf.GradientTape() as tape:
        ...
        for step, (state, action) in enumerate(zip(agent.state_history, agent.action_history)):
            ...
            if steering_power == -1:
                ...
                reward += (20.0*speed_factor if abs(steering_err) < np.pi/90 else -20.0*speed_factor)
...
df.to_csv("training_metrics_nn_steering.csv", index=False)
...
summary_df.to_csv("training_summary_nn_steering.csv", index=False)