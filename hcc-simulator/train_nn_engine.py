...
action_engine = layers.Dense(num_engine_actions, activation="softmax", name = "engine_out",
...
while episode_count < max_episodes:
    ...
    sim.print("Here!")
    agent = agents.NNEngineAgent(model, num_engine_actions)
    sim.print(f"Running engine-only training episode {episode_count + 1}.")
    ...
    with tf.GradientTape() as tape:
        ...
        engine_indicies = tf.constant(agent.action_history, dtype=tf.int32)
        engine_action_probs_history = tf.gather(action_probs, engine_indicies, axis=1, batch_dims=1)
        ...
        for step, (state, action) in enumerate(zip(agent.state_history, agent.action_history)):
            ...
            side_dist_diff_norm = max(-1.0, min(1.0, side_dist_diff/10.0))
            ...
            if engine_power == -1:
                reward += 10.0*speed_err*(side_error_factor + steering_factor) - (0 if speed > 1.0 else 10.0*(speed - 1.0))
            elif engine_power == 0:
                reward += (20.0 if abs(speed_err) < 0.5 else -20.0)
            elif engine_power == 1:
                reward += -10.0*speed_err*(side_error_factor + steering_factor)
            else:
        ...
        # Actor + Critic losses
        actor_losses = -tf.reduce_mean(engine_action_probs_history * diffs)
